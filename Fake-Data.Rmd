...
gender$income <- regression(gender$one , gender$two, gender$three ,alpha,beta)
gender$income
```
```{r}
library(rstan)
...
```{r}
fit1 <- stan(model_code = stan_model, data= list(N=N, K=K, x=subset(gender, select = -income),y=gender$income))
