...
beta <- c(-7,-8,-2)
sigma <- 18
abs(regression_norm(gender$one[1] , gender$two[1], gender$three[1] ,alpha,beta,sigma))
```
...
for (i in 1:400){
 gender$gap[i] <-regression_norm(gender$one[i] , gender$two[i], gender$three[i] ,alpha,beta,sigma)
}
gender$gap
```
